...
def main():
    ...
    sorted_y_values = max_y_values.sort()

    running_total = 0
    times = 0

    for i in range(len(sorted_y_values)):
        if (i == 0 or i == (len(sorted_y_values)) - 1):
            continue
        else:
            running_total += sorted_y_values[i]
            times += 1

    average = running_total / times


    if((sorted_y_values[len(sorted_y_values)] - average) < (average - sorted_y_values[0])):
        outlier = sorted_y_values[len(sorted_y_values)]
    else:
        outlier = sorted_y_values[0]

    for i in range(len(sorted_y_values)):
        if(max_y_values[i] == outlier):
            print(f"Outlier is file number {i + 1} in the directory")

'''
    sorted_y_values = max_y_values.sort()

    if((sorted_y_values[len(sorted_y_values)] - sorted_y_values[len(sorted_y_values)]) < (sorted_y_values[1] - sorted_y_values[0])):
        outlier = sorted_y_values[len(sorted_y_values)]
    else:
        outlier = sorted_y_values[0]
